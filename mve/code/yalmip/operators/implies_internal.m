            ...
            if isa(Y,'sdpvar') | isequal(settype(Y),'elementwise') | isequal(settype(Y),'equality') 
                ...
                switch settype(X)
                    case 'elementwise'
                        ...
                        Guard = set(X <= tol + Mx.*di) ;
                    otherwise
                        error('IMPLIES not implemented for this case');
                end
                
                d = binvar(1);
                Guard = Guard+set(d<=di)+set(d>=(sum(di)-length(di))+1);
                 
                if isa(Y,'sdpvar')
                    varargout{1} = Guard + set(Y>=0.5*(sum(di)-length(di)+1));
                elseif isequal(settype(Y),'elementwise')
                     Y = sdpvar(Y);
                     [My,my]=derivebounds(Y);                                           
                     varargout{1} = Guard + set(Y>=my*d);
                elseif isequal(settype(Y),'equality')
                     Y = sdpvar(Y);
                     [My,my]=derivebounds(Y);                            
                     Guard = Guard + set(Y>=my*(1-d));
                     [My,my]=derivebounds(-Y);                            
                     varargout{1} = Guard + set(-Y>=my*(1-d));
                end
